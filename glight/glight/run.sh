...
NUM_ZONES=5 # Assuming 6 controllable zones on G213
...
DEFAULT_COLORS=("ff00ff"  "0000ff"  "00ff00" "ffff00" "ff0000" "00ffff" ) #Magenta,  Blue, Green, Yellow, Red, Cyan
...
#  echo "Debug (manual_cycle): Received arguments: $@"
  for arg in "$@"; do
#    echo "Debug (manual_cycle): Processing argument: $arg"
    color_hex=$(get_color_hex "$arg")
#    echo "Debug (manual_cycle): Hex for '$arg' is: '$color_hex'"
    if [[ -n "$color_hex" ]]; then
...
#  echo "Debug (manual_cycle): Final color_list: ${color_list[@]}"
...
#    echo "Debug (manual_cycle): shifted_colors: ${shifted_colors[@]}"
    apply_zone_colors "${shifted_colors[@]}"
...
#    echo "Debug (manual_cycle): Rotated color_list: ${color_list[@]}"
  done
}
...
apply_effect() {
  ...
  if [[ -n "$colors_str" ]]; then
    local color_array=(${colors_str})
    manual_cycle "${color_array[@]}"
  else
